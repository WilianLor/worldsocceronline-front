...
const FindCoach: React.FC = () => {
    ...
    useEffect(() => {
        ...
        axios.get(config.baseUrl+`/coaches/true/null/null`, header).then((response: AxiosResponse) => {
        ...
        <Body>
            ...
            <FindCoachPage>
                <Header>
                    ...
                    <FilterContent>
                        <CheckBoxField>
                            <CheckBox type="checkbox" name="interest" defaultChecked={true} onChange={(e) => {changeOnlyInterested(e.target.checked)}}/>
                    ...
                    <PageContent>
                        {showPages().map(page => <PageButton key={page} id={activePage === page ? 'active' : 'disabled'} onClick={() => setActivePage(page)}>{page}</PageButton>)}
                    </PageContent>
                </Header>
                <ListOfCoaches>
                    { coaches.coaches.map((coach: CoachInterface, index) => (
                        (index + 1) / 20 <= activePage && (index + 1) / 20 > activePage - 1 ?
                            
                            <Coach key={coach._id} href={"/userprofile/"+coach._id}>
                                <Username>
                                    {coach.username}
                                </Username>
                                    {coach.teamImage ? 
                                    <Flag src={coach.teamImage}/> :
                                    <ContentText>Sem time</ContentText>}
